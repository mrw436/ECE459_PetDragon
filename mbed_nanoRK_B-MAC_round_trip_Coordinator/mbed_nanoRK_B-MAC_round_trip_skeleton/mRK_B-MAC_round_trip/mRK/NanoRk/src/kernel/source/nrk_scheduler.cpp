...
void _nrk_scheduler()
{
    ...
    //pc4.printf("scheduler called\r\n");
    /* Dalton Banks: why was this here?? changed to _nrk_os_timer_reset() */
...
*/
    ...
    nrk_system_time.nano_secs-=(nrk_system_time.nano_secs%(uint32_t)NANOS_PER_TICK);
		//pc4.printf("nano_secs: %u \r\n",nrk_system_time.nano_secs);
#ifdef NRK_STATS_TRACKER
		//pc4.printf("in tracker");
    ...
    if(nrk_cur_task_TCB->suspend_flag==1 && nrk_cur_task_TCB->task_state!=FINISHED)
    {
        ...
        else
        {
            ...
            nrk_cur_task_TCB->nw_flag=0;
				...
				//pc4.printf("nrk_cur_task_TCB->task_ID: %d\r\n",nrk_cur_task_TCB->task_ID);
    ...
    {
				//pc4.printf("in for, task_id:%d\r\n",task_ID);
        if(nrk_task_TCB[task_ID].task_ID==-1) continue;
        nrk_task_TCB[task_ID].suspend_flag=0;
        if( nrk_task_TCB[task_ID].task_ID!=NRK_IDLE_TASK_ID && nrk_task_TCB[task_ID].task_state!=FINISHED )
        {
            //pc4.printf("next wake_up: %d\r\n",nrk_task_TCB[task_ID].next_wakeup);
					  //pc4.printf("_nrk_prev_timer_val: %d\r\n",_nrk_prev_timer_val);
...
//  If we disable power down, we still need to wakeup before the overflow
    ...
    _nrk_prev_timer_val=next_wake;
		...
		//Madhur: This part screws the first period.
		//pc4.printf("os_timer:%d\r\n",_nrk_os_timer_get());
    ...
    // Bound Context Swap to 100us
		...
		//wait_ms(1);
#endif
		//if(task_ID==NRK_IDLE_TASK_ID)nrk_high_ready_TCB = &nrk_task_TCB[1];
