...
int8_t bmac_set_channel(uint8_t chan)
{
    ...
    g_chan=chan;
    rf_init (&bmac_rfRxInfo, chan, 0xFFFF, MAC_ADDR);
    return NRK_OK;
}
...
int8_t bmac_rx_pkt_set_buffer(char *buf, uint8_t size)
{
...
int8_t bmac_tx_pkt_nonblocking(char *buf, uint8_t len)
{
    if(tx_data_ready==1) return NRK_ERROR;
    bmac_rfTxInfo.pPayload = buf;
    bmac_rfTxInfo.length = len;
		bmac_rfTxInfo.cca = 0;
		bmac_rfTxInfo.ackRequest = 1;
		bmac_rfTxInfo.destAddr = 0xFFFF;	// TODO: Change this from boadcast
    tx_data_ready = 1;
    return NRK_OK;
}
...
char *bmac_rx_pkt_get(uint8_t *len, uint8_t *rssi)
{
...
// To Madhur: these are simplified versions of these three functions, rewritten by me.
// The originals are block-commented out at the bottom of the file.
// Its okay Paul, i will look into it: Madhur
...
//		nrk_wait(_bmac_check_period);
		...
		nrk_time_t carrierTransmissionTime;
		carrierTransmissionTime.secs=_bmac_check_period.secs;
		carrierTransmissionTime.nano_secs=_bmac_check_period.nano_secs + 5*NANOS_PER_MS;
		nrk_wait(carrierTransmissionTime);
    ...
    rf_rx_off();
    tx_data_ready = 0;
    nrk_event_signal (bmac_tx_pkt_done_signal); // SIGNAL
    return NRK_OK;
}
...
int8_t _bmac_rx()
{
    ...
    cnt=0;
    while ((n = rf_rx_check_cca()) != 0)
    {
        cnt++;
			
//		nrk_time_t packetWaitTime;
//		packetWaitTime.secs=0;
//		packetWaitTime.nano_secs=10*NANOS_PER_MS;
//		nrk_wait(packetWaitTime);
...
//		nrk_time_t packetWaitTime;
//		packetWaitTime.secs=0;
//		packetWaitTime.nano_secs=25*NANOS_PER_MS;
//		nrk_wait(packetWaitTime);
...
void bmac_nw_task ()
{
    ...
    // v=nrk_signal_register(bmac_enable_signal);			SIGNAL
        ...
        if(is_enabled )
        {
            ...
            if(v==0)
            {
                ...
                if(_bmac_rx()==1)
                {
                    ...
                    // e=nrk_event_signal (bmac_rx_pkt_signal);		SIGNAL
                    //if(e==NRK_ERROR) {
