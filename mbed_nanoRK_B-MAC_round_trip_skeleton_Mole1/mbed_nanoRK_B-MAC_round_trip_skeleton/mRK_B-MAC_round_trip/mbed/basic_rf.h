...
// TODO: Paul Gurniak, determine if these are needed
...
void rf_addr_decode_enable(); // Vuk: This function doesn't work right and should be used. Use rf_set_rx_mode()
void rf_addr_decode_disable(); // Vuk: This function doesn't work right and should be used. Use rf_set_rx_mode()


//nrk_sem_t* rf_get_sem();  // TODO: Paul Gurniak
...
#define RF_PACKET_OVERHEAD_SIZE   ((2 + 1 + 2 + 2 + 2))     // Do not include checksum/footer for MRF24J40
#define RF_MAX_PAYLOAD_SIZE        (127 - RF_PACKET_OVERHEAD_SIZE - 3) // On the Rx side, however, we still have a 3-byte footer
#define RF_ACK_PACKET_SIZE        5                           // Unused, handled in HW
...
// If rxMode == 0 : NORMAL MODE
// If rxMode == 1 : PROMISCUOUS MODE
// If rxMode == 2 : ERROR MODE
