...
void initDEBUG(void) 
{
    ...
    pinConfig.Pinmode = PINSEL_PINMODE_PULLUP;
    pinConfig.Funcnum = PINSEL_FUNC_0;
    pinConfig.Portnum = NRK_PORTC;
    pinConfig.Pinnum = DEBUG_0;        // DEBUG 0
    PINSEL_ConfigPin(&pinConfig);
    pinConfig.Pinnum = DEBUG_1;        // DEBUG 1
    PINSEL_ConfigPin(&pinConfig);
    pinConfig.Pinnum = DEBUG_2;        // DEBUG 2
    PINSEL_ConfigPin(&pinConfig);
    pinConfig.Pinnum = DEBUG_3;        // DEBUG 3
    PINSEL_ConfigPin(&pinConfig);
    
    /* Enable bits corresponding to DEBUG as outputs. */
    GPIO_SetDir(2, DEBUG_0, 1);
    GPIO_SetDir(2, DEBUG_1, 1);
    GPIO_SetDir(2, DEBUG_2, 1);
    GPIO_SetDir(2, DEBUG_3, 1);
    /* Turn off all MBED_MBED_DEBUGs. */
    GPIO_ClearValue(2, DEBUG_0);
    GPIO_ClearValue(2, DEBUG_1);
    GPIO_ClearValue(2, DEBUG_2);
    GPIO_ClearValue(2, DEBUG_3);
}
    ...
    switch (pin) {
        ...
        case LED_3:
            return (uint8_t (GPIO_ReadValue(1) & (1<<pin)));
				...
				case DEBUG_2:
				case DEBUG_3:
						return (uint8_t (GPIO_ReadValue(2) & (1<<pin)));
        default: return -1;
    }
    return 1;
}
    ...
    switch (pin) {
        ...
        case LED_3:
            GPIO_SetDir(1, pin, pin_direction); break; 
				...
				case DEBUG_2:
				case DEBUG_3:
					GPIO_SetDir(2, pin, pin_direction); break; 
        default: return -1;
    }
