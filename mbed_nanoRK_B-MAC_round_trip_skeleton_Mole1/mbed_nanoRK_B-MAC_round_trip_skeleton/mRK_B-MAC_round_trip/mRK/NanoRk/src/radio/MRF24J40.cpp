...
{
	nrk_int_disable(); // Vuk: To prevent possible interleaving of SPI usage
    mrf_cs_set(0);
    nrk_spin_wait_us(1);
...
void mrf_init(uint8_t channel)
{
    ...
    mrf_reset(channel);                    // Hard and soft reset, configure for Tx/Rx
...
void mrf_reset(uint8_t channel)
{
	...
	// Vuk: Datasheet does not suggest HW reset at this point	
	//    mrf_cs_set(1);
	//    mrf_reset_set(0);
	...
	mrf_write_short(SOFTRST, 0x07);

	//	Vuk: Datasheet does not suggest SW reset at this point	
	//  // Software RF reset
	//  mrf_write_short(RFCTL, 0x04);
	//  mrf_write_short(RFCTL, 0x00);

	mrf_write_short(PACON2, 0x98);
	
	//	Vuk: Datasheet does not suggest FIFO flush at this point	
	//  // Flush Rx FIFO
	//  mrf_write_short(RXFLUSH, 0x01);

	mrf_write_short(TXSTBL, 0x95); // Vuk: This is a recommended value

  // Enable and configure radio
  mrf_write_long(RFCON0, 0x03);   // Vuk: This was changed from 0xE3 to 0x03 as per datasheet
  mrf_write_long(RFCON1, 0x02);   // Set VCO param to 2
  mrf_write_long(RFCON2, 0x80);   // Enable PLL
  //mrf_write_long(RFCON3, 0x00);   // Max Rx/Tx power // Vuk: This is commented since datasheet does not suggest Tx power setting at this point
  ...
  mrf_write_long(RFCON8, 0x10);   // Enable VCO
	...
	mrf_write_long(SLPCON1, 0x21);  // Disable CLKOUT, set slpclk div to 1 // Vuk: this was mistakenly set to 0x20

	//	Vuk: Datasheet does not suggest this setting at this point	
	// mrf_write_short(RXMCR, 0x01);   // Ignore address field

  mrf_write_short(BBREG2, 0x80);  // Set CCA mode to energy detect
  mrf_write_short(CCAEDTH, 0x60); // Set RSSI threshold to recommended
  ...
  mrf_write_long(SLPCON0, 0x02);  // Set interrupt polarity to rising edge
	...
	mrf_write_long(RFCON0, ((channel-11 << 4) | 0x03));   // Vuk: Datasheet suggests setting channel at this point

	//	Vuk: Datasheet does not suggest this setting at this point
  //  // Sleep configuration
  //  mrf_write_short(WAKECON, 0x80); // Enable immediate wake-up
    
	// Vuk: Datasheet suggests setting Tx power at this point
	mrf_write_long(RFCON3, 0x00); // Max Tx power
...
void mrf_test_mode(void)	// Vuk: This function is not used
{
...
void mrf_data_mode(void)	 // Vuk: This function is not used
{
