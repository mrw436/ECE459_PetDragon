...
//Serial pc5(USBTX, USBRX);
extern "C" void _nrk_spin_wait_1us();
void nrk_spin_wait_us(uint32_t timeout)
{
...
void _nrk_setup_timer() {
    ...
    LPC_TIM0->PR = 96000; // set prescale to 93750 (1024 Hz timer)

		//LPC_TIM0->PR = 93658; // 
    
    LPC_TIM0->MR0 = 254; // match0 compare value (8-bit)..only intialization
    LPC_TIM0->MCR |= BM(0)|BM(1); // interrupt and reset on match0 compare
  //  LPC_TIM0->MR1 = 255; // match0 compare value (8-bit)..only intialization
  // LPC_TIM0->MCR |= BM(4); // interrupt and reset on match0 compare
    NVIC_EnableIRQ(TIMER0_IRQn); // enable timer interrupt
    
    
    // set up high precision timer (timer1)
    LPC_SC->PCONP |= BM(2); //power up timer1
    LPC_SC->PCLKSEL0 |= BM(4); // clock = CCLK (96 MHz)
    LPC_TIM1->PR = 12; // set prescale to 12 (8 MHz timer)
    
    LPC_TIM1->MR0 = 65535; // match0 compare value (16-bit)
    //LPC_TIM1->MCR |= BM(0)|BM(1); // interrupt and reset on match0 compare
		LPC_TIM1->MCR |= BM(1); // interrupt and reset on match0 compare
...
{
    volatile uint16_t tmp; // TODO: this seems unnecessary..
    //tmp = LPC_TIM1->TCR;
		tmp = (volatile uint16_t) LPC_TIM1->TC;
    return	tmp;
}
       ...
       do
			 {
				LPC_TIM0->IR |= (1 << 0); // Clear MR0 interrupt flag
				 LPC_TIM0->TC = 0x0; // Just a test..nrk os timer reset was disabled for this..
				 // Updated comment: Madhur: This works ! The timer did not reset by itself, so this is necessary indeed. 
